            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            ...
                            <tbody>
                                @forelse ($contacts as $contact)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="px-6 py-4">{{ $contact->ip_address }}</td>
                                        <td class="px-6 py-4">{{ $contact->created_at }}</td>
                                        <td class="px-6 py-4 flex flex-row gap-1">
                                            <x-secondary-button wire:click="view({{ $contact->id }})">View</x-secondary-button>
                                            <x-danger-button wire:click.prevent="showConfirmationModal({{ $contact->id }})">
                                                Delete
                                            </x-danger-button>
                                        </td>
                                    </tr>
                        ...
                        <x-pagination :collection="$contacts"/>
                    </div>
                </div>
            </div>
        </div>
